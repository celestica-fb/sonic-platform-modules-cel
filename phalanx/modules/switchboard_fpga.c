/*
 * switchboard.c - Driver for Phalanx Switch FPGA/CPLD.
 *
...
#define CLASS_NAME "phalanx_fpga"
#define DRIVER_NAME "AS23128h.switchboard"
...
struct phalanx_fpga_data *fpga_data;
...
static struct device * phalanx_sff_init(int portid) {
 ...
 * performance.
 */
static int i2c_wait_ack(struct i2c_adapter *a, unsigned long timeout, int writing, int *status) {
    int error = 0;
    int Status;
...
static int i2c_wait_stop(struct i2c_adapter *a, unsigned long timeout, int writing, int *status) {
    int error = 0;
    int Status;
                        ...
                        int size, union i2c_smbus_data *data)
{
    ...
    int *smb_pkg_stat = kzalloc(64*sizeof(int), GFP_KERNEL);
    unsigned int *smb_debug_cmd_dat=kzalloc(64*sizeof(unsigned int), GFP_KERNEL);
    int smb_pkg_tai = 0;
    ...
    //// Wait {A}
    // + IACK
    error = i2c_wait_ack(adapter, 30, 1, &smb_pkg_stat[smb_pkg_tai]);
    ...
    if ( rw == I2C_SMBUS_READ && (
        ...
        /* Collect STATUS register before Repeat Start */
        pre_rep_start = ioread8(pci_bar + REG_STAT);
    ...
    if ( rw == I2C_SMBUS_READ && (
        ...
        for (bid = 0; bid < cnt; bid++) {
            ...
            // Wait {A}
            error = i2c_wait_ack(adapter, 30, 0, &smb_pkg_stat[smb_pkg_tai]);
...
Done:
    ...
    error_stop = i2c_wait_stop(adapter, 30, 0, &smb_pkg_stat[smb_pkg_tai]);
    if (error_stop < 0) {
        dev_dbg(&adapter->dev,"STOP Error: %d\n", error_stop);
    }
    int j;
    for (j=0; j<smb_pkg_tai; j++){
         pr_debug("Port_%d  %c--0x%02x, err=%d, status=0x%02x\n",
                   dev_data->portid, smb_pkg_sta[j], smb_pkg_val[j], smb_pkg_ret[j], smb_pkg_stat[j]);
    }
    /* The 'done_status' is the status register when BUSY is cleared in STOP state */
    pr_debug("Port_%d  stop_err=%d, done_status=0x%x, rep_stat=0x%02x, Sr BAR=%p, DAT_ADDR=%p, CMD_ADDR=%p\n", 
             dev_data->portid, 
             error_stop, 
             smb_pkg_stat[smb_pkg_tai],
             pre_rep_start,
             repeat_start[0], 
             repeat_start[1],
             repeat_start[2]);
...
static const struct i2c_algorithm phalanx_i2c_algorithm = {
 ...
 */
static struct i2c_adapter * phalanx_i2c_init(struct platform_device *pdev, int portid, int bus_number_offset)
{
    int error;
    ...
    new_adapter->algo  = &phalanx_i2c_algorithm;
...
// I/O resource need.
static struct resource phalanx_resources[] = {
    {
        .start  = 0x10000000,
        .end    = 0x10001000,
...
static void phalanx_dev_release( struct device * dev)
{
    return;
}

static struct platform_device phalanx_dev = {
    .name           = DRIVER_NAME,
    .id             = -1,
    .num_resources  = ARRAY_SIZE(phalanx_resources),
    .resource       = phalanx_resources,
    .dev = {
        .release = phalanx_dev_release,
    }
};
...
MODULE_AUTHOR("Pradchaya P. <pphuchar@celestica.com> XiaoShen CMD/STAT latch DB_1.8");
