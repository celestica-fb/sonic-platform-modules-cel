...
{ uint32_t res;
  ...
  if (*gpio_num <= 31) {
       ...
       res = (inl_p(GP_LVL) & (1 << *gpio_num)) ? 1 : 0;
	   return res;  
  } else if (*gpio_num <= 63) {
             res = (inl_p(GP_LVL2) & (1 << *gpio_num)) ? 1 : 0;
	         return res;
  	     } else if (*gpio_num <= 75) {
                    res = (inl_p(GP_LVL3) & (1 << *gpio_num)) ? 1 : 0;
	                return res;
  	     	     } else if (*gpio_num > 75) {
  	                       perror("gpio_num > 75 \n");
						               exit(-1);
...
void gpio_write(uint32_t * gpio_num, uint32_t gpio_lvl)
{ uint32_t reg_val;
  ...
  if (*gpio_num <= 31) {
	   ...
	   reg_val = inl_p(GP_LVL);
	   if (gpio_lvl) {
          reg_val |= 1 << *gpio_num;
	   } else {
                reg_val &= ~(1 << *gpio_num);
	   	    }
	   outl_p(reg_val, GP_LVL);
  } else if (*gpio_num <= 63) {
			 reg_val = inl_p(GP_LVL2);
	         if (gpio_lvl) {
                 reg_val |= 1 << *gpio_num;
	         } else {
                     reg_val &= ~(1 << *gpio_num);
	   	          }
	         outl_p(reg_val, GP_LVL2);
		 } else if (*gpio_num <= 75) {
				    reg_val = inl_p(GP_LVL3);
	                if (gpio_lvl) {
                        reg_val |= 1 << *gpio_num;
	                } else {
                             reg_val &= ~(1 << *gpio_num);
	   	                  }
	                outl_p(reg_val, GP_LVL3);
				} else if (*gpio_num > 75) {
  	              perror("gpio_num > 75 \n");
						      exit(-1);
  	           }
...
/*
*The following function init gpio to output HIGH
*
*/
void gpio_init_default(uint32_t * gpio_num)
{ uint32_t reg_val;
  ...
  if (*gpio_num <= 31) {
	   ...
	   reg_val = inl_p(GPIO_USE_SEL);
	   reg_val |= 1 << *gpio_num;
	   outl_p(reg_val, GPIO_USE_SEL);

	   reg_val = inl_p(GP_IO_SEL);
	   reg_val &= ~(1 << *gpio_num);
	   outl_p(reg_val, GP_IO_SEL);
	   
	   reg_val = inl_p(GP_LVL);
	   reg_val |= 1 << *gpio_num;
	   outl_p(reg_val, GP_LVL);
  } else if (*gpio_num <= 63) {
             reg_val = inl_p(GPIO_USE_SEL2);
	         reg_val |= 1 << *gpio_num;
	         outl_p(reg_val, GPIO_USE_SEL2);

	         reg_val = inl_p(GP_IO_SEL2);
	         reg_val &= ~(1 << *gpio_num);
	         outl_p(reg_val, GP_IO_SEL2);
	   
	         reg_val = inl_p(GP_LVL2);
	         reg_val |= 1 << *gpio_num;
	         outl_p(reg_val, GP_LVL2);
  	     } else if (*gpio_num <= 75) {
                    reg_val = inl_p(GPIO_USE_SEL3);
	                reg_val |= 1 << *gpio_num;
	                outl_p(reg_val, GPIO_USE_SEL3);

	                reg_val = inl_p(GP_IO_SEL3);
	                reg_val &= ~(1 << *gpio_num);
	                outl_p(reg_val, GP_IO_SEL3);
...
int32_t gpio_open(uint32_t * gpio_num, int gpio_lvl)
{
  return 0;
}
void gpio_close(uint32_t * gpio_num)
{
  return;
}
int32_t gpio_change_direction(uint32_t * gpio_num, uint32_t gpio_dir)
{ uint32_t reg_val;
  ...
  if (*gpio_num <= 31) {
	   ...
	   reg_val = inl_p(GP_IO_SEL);
	   if (gpio_dir == GPIO_DIRECTION_OUT) {
	       reg_val &= ~(1 << *gpio_num);
	       outl_p(reg_val, GP_IO_SEL);
	   } else {
               reg_val |= 1 << *gpio_num;
...
int main(int argc, char* const argv[])
{
  ...
  if (iopl(3)) {
     perror("iopl privilege unavailable");
     exit(-1);
  }
  outl_p(0x0, GP_BLINK);
  /* open all gpio */
