/*
 * switchboard.c - Driver for Phalanx2 Switch FPGA/CPLD.
 *
...
#define FPGA_PCI_NAME "phalanx2_fpga_pci"
...
/*
 * struct phalanx2_fpga_data - Private data for phalanx2 switchboard driver.
...
struct phalanx2_fpga_data *fpga_data;
...
static struct device * phalanx2_sff_init(int portid) {
 ...
 *
 * Phalanx2 front panel port mapping shown below.
...
static int i2c_xcvr_access(u8 register_address, unsigned int portid, u8 *data, char rw){
    ...
    /* Normalize the port position into 4 x 8 metric.
    ...
    // Select port
    err = fpga_i2c_access(fpga_data->i2c_adapter[i2c_adapter_index],
                          dev_addr,
                          0x00,
                          I2C_SMBUS_WRITE,
                          I2C_XCVR_SEL,
                           ...
                           int size, union i2c_smbus_data *data)
{
    ...
    if(retry <= 0 && error == -EBUSY ){
        ...
        /**
         * Phalanx2: Device specific I2C reset topology
         */
...
static const struct i2c_algorithm phalanx2_i2c_algorithm = {
 ...
 */
static struct i2c_adapter * phalanx2_i2c_init(struct platform_device *pdev, int portid, int bus_number_offset)
{
    int error;
    ...
    new_adapter->algo  = &phalanx2_i2c_algorithm;
...
// I/O resource need.
static struct resource phalanx2_resources[] = {
    {
        .start  = 0x10000000,
        .end    = 0x10001000,
...
static void phalanx2_dev_release( struct device * dev)
{
    return;
}

static struct platform_device phalanx2_dev = {
    .name           = DRIVER_NAME,
    .id             = -1,
    .num_resources  = ARRAY_SIZE(phalanx2_resources),
    .resource       = phalanx2_resources,
    .dev = {
        .release = phalanx2_dev_release,
    }
};
...
static int phalanx2_drv_probe(struct platform_device *pdev)
{
    struct resource *res;
    int ret = 0;
    int portid_count;
    ...
    fpga_data = devm_kzalloc(&pdev->dev, sizeof(struct phalanx2_fpga_data),
                             GFP_KERNEL);
    ...
    for (portid_count = 0 ; portid_count < VIRTUAL_I2C_PORT_LENGTH ; portid_count++) {
        ...
        fpga_data->i2c_adapter[portid_count] = phalanx2_i2c_init(pdev, portid_count, VIRTUAL_I2C_BUS_OFFSET);
    }
    ...
    for (portid_count = 0; portid_count < SFF_PORT_TOTAL; portid_count++) {
        ...
        if (i2c_adap) {
            fpga_data->sff_devices[portid_count] = phalanx2_sff_init(portid_count);
...
static int phalanx2_drv_remove(struct platform_device *pdev)
{
    int portid_count;
...
static struct platform_driver phalanx2_drv = {
    .probe  = phalanx2_drv_probe,
    .remove = __exit_p(phalanx2_drv_remove),
    .driver = {
        .name   = DRIVER_NAME,
    },
};
...
static int fpga_pci_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    platform_device_register(&phalanx2_dev);
    platform_driver_register(&phalanx2_drv);
    return 0;
...
{
    platform_driver_unregister(&phalanx2_drv);
    platform_device_unregister(&phalanx2_dev);
    fpgafw_exit();
...
void phalanx2_exit(void)
{
    pci_unregister_driver(&pci_dev_ops);
}

module_init(phalanx2_init);
module_exit(phalanx2_exit);
...
MODULE_DESCRIPTION("Celestica phalanx2 switchboard platform driver");
